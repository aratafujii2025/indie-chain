import * as React from 'react';
import {useNavigate} from 'react-router-dom';
import axios from 'axios';
import Header from '../../components/Header';
import Copy from '../../icons/Copy';
import Upload from '../../icons/Upload';
import DisplayText from '../../components/DisplayText';
import Verify from './Verify';
import OTC from './OTC';
import ChangePw from './ChangePw';

function EditAccount() {
    const navigate = useNavigate();
    const navAcc = () => {
        navigate('/account');
    }

    const[verifyVis, setVerifyVis] = React.useState(false);
    const[otcVis, setOtcVis] = React.useState(false);
    const[changePw, setChangePw] = React.useState(false);

    const [newUsername, setNewUsername] = React.useState("");
    const [newEmail, setNewEmail] = React.useState("");

    const API_URL = "http://localhost:5173/";

    // dummy values for display purposes, initialize to empty strings and 0?
    const [user, setUser] = React.useState({
        username: "lizgran28",
        firstName: "Elizabeth", 
        lastName: "Grant",
        email: "elizabeth.grant@gmail.com",
        id: 4459,
        mmAddr: "0x1223...fe85",
        url: "data:image/jpeg;base64,"
    })

    async function getUser(){
        try{
            const response = await axios.get(API_URL);
            // need to access and set each property individually? 
            setUser(response.data);
        }catch(error){
            console.log(error);
        }
    }
    getUser();

    async function send(idNum:number, newUname:string, newEm:string){
        try{
            const res = await axios.post(
                API_URL, 
                {id: idNum, newUsername: newUname, newEmail: newEm}
            )
        }catch(error){
            console.log(error);
        }
    }

    // controls when to show popups for changing password process
    var verifyZInd: string = "-z-10"
    if(verifyVis){
        verifyZInd = "z-10"
    }
    var verifyClass:string = verifyZInd.concat(" absolute w-full h-full");

    var otcZInd: string = "-z-20"
    if(otcVis){
        otcZInd = "z-20"
    }
    var otcClass:string = otcZInd.concat(" absolute w-full h-full");

    var pwZInd: string = "-z-30"
    if(changePw){
        pwZInd = "z-30"
    }
    var pwClass:string = pwZInd.concat(" absolute w-full h-full");

    return (
        <div>
            <div className="bg-cream w-full h-full overflow-auto flex flex-nowrap flex-col z-0 absolute">
                <div> <Header/> </div>
                <h1 className="text-b3 left font-bold italic mx-16 my-5">Account</h1>

                <div className="bg-white w-[64rem] h-[50rem] rounded-3xl mb-10 ml-52">
                    <div className="w-1 h-1 bg-transparent bg-opacity-0 ml-[36rem]">
                        <button 
                            onClick={()=>""}
                            className="bg-pink/50 w-80 h-9 rounded-full text-center text-white mt-8">
                                Reconnect Wallet
                        </button>
                    </div>
                
                    <div className="inline-block ml-28 mr-2 mt-5">
                        <div className="relative z-20 bg-transparent bg-opacity-0 w-1 h-1">
                            <div className="absolute top-14 left-14">
                                <Upload color="#FFFFFFF"/>
                            </div>
                        </div>
                        <div className="-mt-1">
                            <img 
                                className="w-36 h-36 rounded-full opacity-60"
                                src={user.url}
                                alt="user profile picture"
                            />
                        </div>
                        <div>
                        </div>
                    </div>

                    <div className="inline-block w-[35rem]">
                        <h3 className="font-semibold italic pt-2">Hi {user.firstName}</h3>
                    </div>

                    <button 
                        onClick={()=> {
                            send(user.id, newUsername, newEmail);
                            navAcc();
                        }}
                        type="submit" 
                        className="inline-block w-7 h-7 ml-10 -mb-2 opacity-50"> 
                            Save
                    </button>
 
                    <div className="flex flex-nowrap flex-col items-center mb-6">
                        <div className="py-4 px-6 mb-2 w-[50rem] h-36 mt-2 rounded-3xl border-2 border-silver-light">
                            <h4 className="italic font-semibold mb-4">Profile</h4>

                            <div className="mb-2">
                                <label className="inline-block pl-4 pr-8 text-silver-dark">Username</label>
                                <input 
                                onChange={(e) => setNewUsername(e.target.value)}
                                type="text" defaultValue={user.username} 
                                className="inline-block text-b3 hover:outline-none hover:ring focus:outline-silver-light outline-1 pl-2 rounded-full pr-2"/>
                            </div>

                            <div className="mb-2">
                                <div className="inline-block pl-4 pr-8 text-silver-dark">
                                    Password
                                </div>
                                <div onClick={() => setVerifyVis(true)} className="inline-block hover:ring rounded-full pl-2 pr-28 text-b3">
                                    ••••••••••••••••
                                </div>
                            </div>

                        </div>

                        <div className="py-4 px-6 mb-2 w-[50rem] h-36 mt-2 rounded-3xl border-2 border-silver-light">
                            <h4 className="italic font-semibold mb-4">Personal</h4>
                            
                            <div className="columns-2">
                                <DisplayText label={"First Name"} text={user.firstName} color="silver-dark"/>
                                <DisplayText label={"Last Name"} text={user.lastName} color="silver-dark"/>
                                
                                <div className="mb-2">
                                    <label className="inline-block pl-4 pr-8 text-silver-dark">Email</label>
                                    <input 
                                    onChange={(e) => setNewEmail(e.target.value)}
                                    type="text" defaultValue={user.email} 
                                    className="inline-block text-b3 hover:outline-none hover:ring focus:outline-silver-light outline-1 pl-2 rounded-full pr-2"/>
                                </div>
                                <DisplayText label={"Phone"} text="Coming Soon!" color="silver-dark"/>
                            </div>
                        </div>

                        <div className="py-4 px-6 mb-2 w-[50rem] h-36 mt-2 rounded-3xl border-2 border-silver-light">
                            <h4 className="italic font-semibold mb-4 text-silver-dark">Details</h4>
                            <DisplayText label={"Arpeggio User ID"} text={user.id.toString()} color="silver-dark"/>
                            <DisplayText label={"MetaMask Address"} text={user.mmAddr} color="silver-dark"/>
                        </div>
                    </div>
               </div> 
            </div>

            <div className={verifyClass}>
                <Verify visible={false} setVisible={setVerifyVis} setNext={setOtcVis} email={user.email}/>
            </div>

            <div className={otcClass}>
                <OTC visible={false} setPrev={setVerifyVis} setVisible={setOtcVis} setNext={setChangePw}/>
            </div>

            <div className={pwClass}>
                <ChangePw visible={false} setVisible={setChangePw} username={user.username}/> 
            </div>
        </div>
        
    )
}

export default EditAccount